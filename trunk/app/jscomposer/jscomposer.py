...
SCRIPT_INC_PATTERN = r'<script src="/scripts/%s-%d-%d.js"></script>'
SCRIPT_URL_PATTERN = r'^scripts/(?P<name>%s)(-(?P<version>[0-9]+)-(?P<debug>[01]))?.js$'
...
class ScriptIncludes():
    ...
    def __getattr__(self, alias):
        if settings.SCRIPT_COMBINE or alias not in settings.SCRIPT_ALIASES:
            return SCRIPT_INC_PATTERN % (alias, settings.SCRIPT_VERSION, settings.SCRIPT_DEBUG)
...
def ScriptPattern(name=None):
    ...
    if name is None:
        name = r'[a-zA-Z0-9_]+'
    logging.info(SCRIPT_URL_PATTERN %name)
...
def ScriptFile(req, debug=None, version=None, name=None, files=None):
    ...
    logging.info("debug: %s, version: %s, name: %s, files: %r" %
                 (debug, version, name, files))
    ...
    sMemKey = 'jscompose-%s-%d-%d' % (name, version, debug)
    ...
    if sScript is None:
        ...
        if version != settings.SCRIPT_VERSION:
            raise Http404
        sMemKey = 'jscompose-%s-%d-%d' % (name, settings.SCRIPT_VERSION, debug)
        sScript = ''    
        for name in files:
            if debug:
                sScript += "/* ---------- %s.js ---------- */\n" % name
        ...
        if settings.SCRIPT_CACHE:
            ...
            memcache.set(sMemKey, sScript, 60*60)
